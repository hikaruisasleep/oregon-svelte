...
public class ImGuiController : IDisposable
{
    ...
    private void UpdateImGuiInput(InputSnapshot snapshot)
    {
        ...
        for (var i = 0; i < snapshot.KeyCharPresses.Count; i++) io.AddInputCharacter(snapshot.KeyCharPresses[i]);
    ...
    private void RenderImDrawData(ImDrawDataPtr draw_data, GraphicsDevice gd, CommandList cl)
    {
        ...
        for (var n = 0; n < draw_data.CmdListsCount; n++)
        {
            ...
            for (var cmd_i = 0; cmd_i < cmd_list.CmdBuffer.Size; cmd_i++)
            {
                ...
                cl.DrawIndexed(pcmd.ElemCount, 1, pcmd.IdxOffset + (uint)idx_offset, (int)pcmd.VtxOffset + vtx_offset,
                    0);
            }
